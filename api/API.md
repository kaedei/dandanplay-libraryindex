...
- method：以下取值之一，分别代表 播放、停止、暂停、下一个视频、上一个视频
  - play
  - stop
  - pause
  - next
...
- Duration：视频时长，单位为秒
- LastThumbnail：上次生成缩略图的时间
- IsStandalone：是否为独立文件，即不包含在媒体库监视文件夹内的文件

## 9.获取视频缩略图 /api/v1/image/{hash} 或 /api/v1/image/id/{id}
...
## 10.控制播放器加载文件 /api/v1/load/{hash} 或 /api/v1/load/id/{id}
...
## 11.获取指定视频的弹幕/api/v1/comment/{hash} 或 /api/v1/comment/id/{id}
...
## 12.串流视频 /api/v1/stream/{hash} 或 /api/v1/stream/id/{id}
